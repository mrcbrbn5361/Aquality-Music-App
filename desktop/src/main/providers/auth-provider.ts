// ytmdesktop2/src/main/trpc/routers/auth uyarlaması — Aquality Music tasarımıyla
import { randomUUID } from 'crypto';
import Store from 'electron-store';
interface Client { appId:string; appName:string; appVersion?:string; token:string; createdAt:number; }
const store = new Store<{clients: Client[], pending:any[]}>({ name:'aquality-music-auth-clients', defaults:{ clients:[], pending:[] }});
